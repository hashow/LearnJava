...
by a call to remove() or poll(). In a FIFO queue, all new elements are inserted at the tail of the queue.
...
queue is a function of the queue's ordering policy,
which differs from implementation to implementation. The remove() and poll() methods differ only in their behavior
...
BlockingQueue interface, which extends this interface.

Queue implementations generally do not allow insertion of null elements, although some implementations,
such as LinkedList, do not prohibit insertion of null. Even in the implementations that permit it,
null should not be inserted into a Queue, as null is also used as a special return value by the poll method
to indicate that the queue contains no elements.

Queue implementations generally do not define element-based versions of methods equals and hashCode
but instead inherit the identity based versions from class Object, because element-based equality is not
always well-defined for queues with the same elements but different ordering properties.

This interface is a member of the Java Collections Framework.
*/
public class UsesOfQueue {
...
2. peek() method: Retrieves, but does not remove, the head of this queue, or returns null if this queue is empty.
 */
        System.out.println("----Useful methods for Queue Interface---- ");
        System.out.println(name.element());
        System.out.println(name.poll()); // returns--the head of this queue which is in this case "Jacob", or null if this queue is empty

        //using remove() method to make the head empty
        System.out.println(name.remove());//head("Jacob") should be removed and now head is "Rac"

        System.out.println(name.peek());

        System.out.println(name.removeAll(name));

        System.out.println("----All elements are removed from the list---- ");
        System.out.println(name.peek()); //returns-- null if no elements available

        System.out.println(name.poll()); //returns-- null if no elements available
        System.out.println(name.element()); //NoSuchElementException
    }
}
